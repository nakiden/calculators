...
//Вызывается по нажатию на цифру, передает ее значение в поле вывода
function digitClick(tag){
	...
	if(operant!=null){
		...
		if(!bools)if(result!=null || secondVariable!=null){
			bools = true; secondVariable = null;
		...
		if(!bools)if(result!=null){
			bools = true; firstVariable= null; secondVariable = null;
		}
...
//Вызывается по нажатию на = , выполняет операции
function operationEqual(){
	...
	if (secondVariable == null && result == null){
		secondVariable = firstVariable;
		}else{
			if(!result==0 && isNull==false){
		...
		case '/': 
			...
			if ( secondVariable != 0){
				...
				document.getElementById("resultWindow").value = "error"; result = 0;
			}
...
//Вызывается по нажатию на оператор для немедленного результата , выполняет операции
function imEqual(op){
	document.getElementById("operantWindow").value = null;
...
// Вызывается при нажати на Clear. Очищает все. Освобождает переменные
function clearClick(){
	document.getElementById("operantWindow").value = null;
	minus = false;
	isNull = true;
